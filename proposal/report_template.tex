...
\section{Deep Reinforcement Learning}
 ...
 In this proposal I intend to apply a hybrid approach for RL called Deep Deterministic Policy Gradients (DDPG). In DDPG it is used non-linear action-value function to obtain the expected reward, parametrized by $\theta^Q$ and is obtained by minimizing the loss:
...
In DDPG, agent's policy is expressed by a parametrized function $\mu(s|\theta^\mu)$
